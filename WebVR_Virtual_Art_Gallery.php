...
<html>
  <head>
    <meta charset="utf-8">
    <title>WebVR Virtual Art Gallery</title>
    <meta name="description" content="A-frame Virtual Reality Art Gallery Demo in a Park. By Michael McAnally, June 19, 2019. Dual controller compatible for teleport movement and raycaster click selection launches. Integration Note: Presently for e-commerce use Firefox browser.  Artist data loaded from Database using PHP/MySQL on server-side."></meta>
...
<!-- <script src="https://cdn.jsdelivr.net/gh/aframevr/aframe@3d029a4e746c844fe35ba27e5ff1a6db2b1a1978/dist/aframe-master.min.js"></script>   -->
 ...
 <script src="https://rocketvirtual.com/aframe-master_9.2_old3/dist/aframe-master.min.js"></script>     
    ...
    <script src="https://rocketvirtual.com/aframe-teleport-extras-master/dist/aframe-teleport-extras.min.js"></script>
    <script src="https://rocketvirtual.com/superframe-master/components/event-set/dist/aframe-event-set-component.min.js"></script>  
    ...
    <!--  slice9: Create transparent panels and backgrounds -->
    <script src="https://rocketvirtual.com/aframe-slice9-component-master/dist/aframe-slice9-component.min.js"></script>
...
//*************************************************************************************************************************
//NOTE: you will "replace these with your username and password" information.  Mine has been removed for security purposes.
//Also in $sql statements below you will replace the "your_database_name." part with the name of your own database
//*************************************************************************************************************************
...
for ($curate = 1; $curate <= 7; $curate++) {
    ...
    $sql = "SELECT Dir, File FROM your_database_name.Artwork WHERE Artist_ID = '" .$id1. "' AND Gallery_ID = '" .$id2. "' AND Curate_ID = ' .$curate. '";
    $result = $conn->query($sql);
...
?>
    ...
    <!-- Display the Gallery  -->
    <a-entity gltf-model="#artGallery" scale="1 1 1" position="-18.826 0.006 19.083" rotation="0 0 0" nav-agent="speed: 1.5; active: true" ></a-entity>

    <!-- nav-mesh working  -->
    <a-entity gltf-model="../gltf/navmesh.gltf" scale="" position="0.04732 0.006 -0.0117" rotation="" visible="false" nav-mesh=""></a-entity>

    <!-- Name of the Gallery   -->
    <a-entity id="galleryLettering" text-geometry="value: Virtual Art Gallery"; font: #exoFont; bevelEnabled: true; bevelSize: 0.05; bevelThickness: 0.05; curveSegments: 12; size: .11; height: 0;" material="color:black; metalness:3; roughness: 0; sphericalEnvMap: #blue; transparent: true; opacity: 1" position="3.86855 1.671 -2.85655" scale="0.3 0.3 0.3" rotation="0 99.63 0" ></a-entity>

    <!-- Create the Park environment -->
    <a-entity id="outside" environment="preset: forest; shadow: true; dressingAmount: 375; fog: 0.745; horizonColor: #dbdedb; playArea: 1.25;" visible=""></a-entity>
  ...
  <!-- Setup the camera and controller hands, raycaster and teleport  -->

  <a-entity id="cameraRig" position="-5 0.006 3" movement-controls="constrainToNavMesh: true;">
      <a-entity id="cam" camera look-controls="pointerLockEnabled: true;" wasd-controls mouse-cursor >
          <a-entity id="mouseCursor" cursor="rayOrigin: mouse"></a-entity>
    </a-entity>
        <a-entity id="left" teleport-controls="cameraRig: #cameraRig; button: trigger;" tracked-controls hand-controls="left" movement-controls="constrainToNavMesh: true;" visible="true"></a-entity>
        <a-entity id="right" tracked-controls hand-controls="right" laser-controls raycaster="showLine: true; far: 20; interval: 0; objects: .clickable, a-link" line="color: lawngreen; opacity: 0.5" visible="true"></a-entity>
  </a-entity>
...
//Also in $sql statements below you will replace the "your_database_name." part with the name of your own database
//*************************************************************************************************************************
$username = "replace_with_your_password";
$password = "replace_with_your_password";
...
for ($curate = 1; $curate <= 7; $curate++) {
    ...
    // Select from Curate
    $sql = "SELECT * FROM your_database_name.Curate WHERE Curate_ID = '" .$curate. "' AND Gallery_ID = '" .$id2. "'";
    $result = $conn->query($sql);
    ...
    // Select from Artwork
    $sql = "SELECT * FROM your_database_name.Artwork WHERE Artist_ID = '" .$id1. "' AND Gallery_ID = '" .$id2. "' AND Curate_ID = '" .$curate. "'";
    $result = $conn->query($sql);
